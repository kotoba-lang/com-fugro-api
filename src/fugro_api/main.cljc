(ns fugro_api.main
  "Kotodama WASM entrypoint for the Fugro_api clean-room actor (L4) — Clojure port.
  ...
  facts (`fugro_api.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "fugro_api")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"fugro_api.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "fugro_api-compat" :tier tier :entities entities} 200])
...
(defn start! [] :fugro_api-compat/ready)
